...
def crop_yolo(img_path, format="jpg", h_slice=1, w_slice=1):
	...
	for _i, (_lt, _rb) in enumerate(list(zip(lt, rb))):
		...
		if not os.path.exists(os.path.join(os.path.dirname(img_path), "yolo", "images")):
			os.makedirs(os.path.join(os.path.dirname(img_path), "yolo", "images"))
...
def crop_voc(image_path, format="jpg", h_slice=1, w_slice=1):
	...
	if not os.path.exists(os.path.join(os.path.dirname(image_path), "voc", "labels")):
		os.makedirs(os.path.join(os.path.dirname(image_path), "voc", "labels"))
